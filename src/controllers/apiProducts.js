...
export async function getRandomProducts(req, res) {
    ...
    const query = productcategories.length > 3 ? await Product.findAll({ where: { id: list_of_id }, order: sequelize.random(), limit: 8}) : await Product.findAll({ order: sequelize.random(), limit: 8 }) 
